<template>
  <div class="card"> <!-- LA PAGE EST CLEAN -->
        ...
        <router-link :to="{ name: 'Comment' , params: {id: message.id }}" v-for="message in messages" :key="message.id" id="fil">
            <h2 aria-label="titre de la publication" id="titre"> {{ message.title }} </h2>
            <img :src=" message.attachment "  aria-label="image de la publication" id="image">
            <span aria-label="description de la publication" id="description"> {{ message.content }} </span>
            <div class="item_bottom">
                <p aria-label="date de la publication et nom de la personne qui a publié" id="date_post"> {{ message.User.username }} le {{ formatDate(message.createdAt) }} </p>
                <div id="icon_post">
                    <router-link :to="{ name: 'Comment' , params: {id: message.id }}" class=" comment_icon far fa-comment-dots" aria-label="Les commentaires"></router-link>
                    <router-link to="/homepage" v-if="user.isAdmin == 1 || user.id == message.userId" @click="deleteMessage()" class="fas fa-trash" aria-label="Supprimer la publication" id="icon"></router-link>
                </div>
            </div>
        </router-link>
    </div>
